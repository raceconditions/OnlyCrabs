...
def is_logged_in():
    return True
    x_forwarded_host = request.headers.get('X-Forwarded-Server', request.remote_addr)
    is_remote = x_forwarded_host == 'rosievision.raceconditions.net'
    if not is_remote:
        print('local')
        return True
...
@google_api.route('/google/logout')
@no_cache
def logout():
